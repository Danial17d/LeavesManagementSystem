<x-auth-layout>
    ...
    <div class="max-w-7xl mx-auto px-4 py-10 space-y-8">
        ...
        @if (! auth()->user()->is_return)
            <div class="bg-gray-800 rounded-xl p-5 ring-2 ring-orange-500/40 flex flex-col sm:flex-row sm:items-center gap-4">
                <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-orange-500/20 border border-orange-500/40">
                    <svg class="w-5 h-5 text-orange-400" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-6a2.25 2.25 0 0 0-2.25 2.25v13.5A2.25 2.25 0 0 0 7.5 21h6a2.25 2.25 0 0 0 2.25-2.25V15M12 9l-3 3m0 0 3 3m-3-3h12.75"/>
                    </svg>
                </div>
                <div class="flex-1">
                    <p class="text-white font-semibold text-sm">You are currently marked as on leave.</p>
                    <p class="text-slate-400 text-xs mt-0.5">If you have returned, please confirm your return so your status is updated.</p>
                </div>
                <form method="POST" action="{{ route('user-status.update') }}" class="shrink-0">
                    @csrf
                    <button type="submit"
                            class="inline-flex items-center gap-2 px-4 py-2 bg-orange-500 text-white text-sm font-semibold rounded-lg hover:bg-orange-600 transition">
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor">
        ...
        @if ($adminStats && (! $setupChecklist || $setupChecklist['allDone']))
            @if ($adminStats)
                        ...
                        <div class="flex items-center justify-between mb-5">
                            ...
                            @can(\App\Enums\PermissionType::LeaveRequestList)
                                <a href="{{ route('leave-requests.index') }}"
                                   class="text-sm text-blue-400 hover:text-blue-300 transition">View all â†’</a>
                            @endcan
                        </div>
                                        ...
                                        <div class="flex items-center gap-3 min-w-0">
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <p class="text-slate-400 text-xs mt-0.5">
                                                    {{ $req->leaveType?->name }}
                                                    &middot;
                                                    {{ \Carbon\Carbon::parse($req->from)->format('M d') }}â€“{{ \Carbon\Carbon::parse($req->to)->format('M d, Y') }}
                                                </p>
                                            </div>
                                        </div>
                ...
                @if ($adminStats['managedStructure'])
                    @php $ms = $adminStats['managedStructure'];
                @endphp
                    <div class="bg-gray-800 rounded-xl p-6">
                        <div class="flex items-center justify-between mb-5">
                                ...
                                <div>
                                    ...
                                    @if ($ms->parent)
                                        <p class="text-slate-500 text-xs mt-0.5">
                                            ...
                                            <span class="mx-1 text-slate-600">â€º</span>
                            ...
                            <span class="inline-flex items-center rounded-md px-2.5 py-1 text-xs font-semibold bg-gray-700 text-slate-300 border border-gray-600">
                                ...
                                @can(\App\Enums\PermissionType::StructureView)
                                    <a href="{{ route('structures.show', $ms) }}"
                                       class="text-sm text-blue-400 hover:text-blue-300 transition">View â†’</a>
                                @endcan
                            </div>
                        </div>
        ...
        @if ($employeeStats)
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <div class="bg-gray-800 rounded-xl p-6">
                        <h2 class="text-base font-bold text-white mb-5">Leave Balances â€” {{ now()->year }}</h2>
                ...
                <div class="bg-gray-800 rounded-xl p-6">
                    <div class="flex items-center gap-3 mb-6">
                        ...
                        <div>
                            <h2 class="text-base font-bold text-white">Payroll â€” {{ now()->year }}</h2>
                                ...
                                @foreach ($employeeStats['payrolls'] as $pr)
                                    ...
                                    <tr class="{{ $isCurrentMonth ? 'bg-violet-500/5' : '' }} transition hover:bg-gray-700/20">
                                        ...
                                        <td class="py-3 px-4 text-right tabular-nums {{ $pr->unpaid_deduction > 0 ? 'text-red-400' : 'text-slate-500' }}">
                                            {{ $pr->unpaid_deduction > 0 ? '-'.number_format($pr->unpaid_deduction, 2) : 'â€”' }}
                                        </td>
